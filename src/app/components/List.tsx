...
const List: FC<ListProps> = ({
  ...
  return (
    <div
      className={`grid grid-cols-[minmax(250px,_1.5fr)_repeat(4,_1fr)_100px] px-8 py-5 outline outline-2 outline-gray-100 ${
        is_ ? "bg-[#FAFBFB]" : null
      }`}
    >
      <div className="flex">
        <input
          aria-label="none"
          type="checkbox"
          className="appearance-none self-center w-6 h-6 border-gray-400 border rounded-md
      ...
      <div
      // className={`${pth == "/employeeManagement" ? "self-center" : "hidden"}`}
      >
        <p>{type}</p>
      </div>
